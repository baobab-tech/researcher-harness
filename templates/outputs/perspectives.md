# [Question]: perspectives

**Retrieved:** YYYY-MM-DD · [Index](../_index.md) · [Claims](../claims.md)

## Positions

### [Position, stated as its holders would state it]

**Held by:** [actors, with interests and funding]
**Strongest argument:**
**Evidence cited:** [claim IDs, with status]
**Weakest point:**

## Where they disagree

| Point | Position A | Position B | Kind (empirical / definitional / values) | What would settle it |
|-------|------------|------------|------------------------------------------|----------------------|

## Common ground

## Unsupported claims

[Claims made by any side with status `unsupported` in claims.md.]

## Human review

[Checkpoints run, claims a person confirmed out of those cited, assumed decisions still open.]
